// Lee un link de propiedad y extrae sus metadatos, en este orden de
// prioridad: 1) Open Graph / Twitter Card (título, imagen, descripción),
// 2) datos estructurados JSON-LD (schema.org) para precio, m² y recámaras,
// y 3) como último recurso, un lector de texto en español que busca esos
// mismos datos escritos en prosa dentro de la descripción o el resto de la
// página (ej. "departamento de tres recámaras", "120 m2", "$1,850,000").
// Corre en el servidor de Vite (dev y preview) para evitar el bloqueo de CORS.
// Misma lógica que supabase/functions/fetch-og — esta copia corre localmente
// sin necesitar ninguna cuenta; cuando conecten Supabase, esa Edge Function
// hace lo mismo pero disponible fuera de la red local.
...
// --- JSON-LD (schema.org) ---------------------------------------------
// Muchos portales incrustan un bloque de datos estructurados pensado para
// que Google los lea, con el precio, m² y recámaras en formato limpio.
...
// Último recurso cuando el portal no publica nada estructurado: busca los
// mismos datos escritos en prosa ("tres recámaras", "120 m2", "$1,850,000").
...
export async function fetchOg(url: string): Promise<ListingInfo> {
  const res = await fetch(url, {
    headers: {
      'User-Agent':
        'Mozilla/5.0 (compatible; CasaRatingBot/1.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120 Safari/537.36',
